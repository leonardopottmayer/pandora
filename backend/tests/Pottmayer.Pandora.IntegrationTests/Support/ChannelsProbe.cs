...
/// <summary>Reads the <c>channels.chn006_notification</c> table directly, for asserting on persisted state.</summary>
internal sealed class ChannelsProbe(string connectionString)
{
    ...
    public async Task<int> CountAsync()
    {
        ...
        cmd.CommandText = "SELECT count(*) FROM channels.chn006_notification";
    ...
    public async Task<string> GetActivationTokenAsync(string recipient)
    {
        await using var conn = await OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT payload->>'token' FROM channels.chn006_notification WHERE recipient = $1";
        cmd.Parameters.AddWithValue(recipient);

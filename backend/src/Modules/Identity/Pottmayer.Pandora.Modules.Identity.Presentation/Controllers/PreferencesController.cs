...
using Pottmayer.Pandora.Modules.Identity.Application.Commands.UpsertPreferences;
using Pottmayer.Pandora.Modules.Identity.Application.Queries.GetPreferences;
...
[Authorize]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/identity/preferences")]
    ...
    IUserContextAccessor<UserData> userContextAccessor) : ControllerBase
{
    ...
    {
        var userId = userContextAccessor.Context.User!.Id;
        var command = new UpsertPreferencesCommand(new UpsertPreferencesInput(
            userId, request.Theme, request.Language,
            request.TimeZone, request.WeekStartsOn, request.DefaultAlertOffsetMinutes));

...
-- Structured bank code (COMPE) so the pipeline resolves the layout from the destination's bank.
ALTER TABLE finances.fin012_import_layout
ADD CONSTRAINT ck_fin012_bank_code
...
-- Resolution key for system layouts: one layout per (bank, format, account type).
CREATE UNIQUE INDEX uq_fin012_system_bank_route
ON finances.fin012_import_layout (bank_code, file_format, account_type)
WHERE user_id IS NULL AND bank_code IS NOT NULL;
...
INSERT INTO finances.fin012_import_layout
    (layout_code, name, bank_name, bank_code, file_format, account_type, config)
VALUES (
    'viacredi-ofx',
    'Viacredi — Bank Account (OFX)',
    'Viacredi',

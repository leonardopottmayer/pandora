...
-- write and processing replays instead of losing the update. The update id is unique per bot, not per
-- provider, so the idempotency guard and the offset are keyed by (provider, bot).
CREATE TABLE channels.chn004_inbound_update (
	id uuid NOT NULL DEFAULT uuid_generate_v7(),
	provider VARCHAR(20) NOT NULL,
	-- The specific inbound endpoint within the provider -- for Telegram, the bot name.
...
-- The idempotency guard. The plan called for this triple as the primary key; it is a unique index
-- instead so the table keeps the uuid_generate_v7() surrogate every other table here has.
CREATE UNIQUE INDEX uq_chn004_provider_bot_update
ON channels.chn004_inbound_update (provider, bot, provider_update_id);

-- "What is the highest update we have seen for this bot?" -- the long-polling offset on startup.
CREATE INDEX ix_chn004_provider_bot_update_id_desc
ON channels.chn004_inbound_update (provider, bot, provider_update_id DESC);
